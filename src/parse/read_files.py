...
class PhosphoProtData(Data2):
    ...
    def filter(self, gene: list[str], tests: list[str], stats: dict[str, bool]):
...
def load_processed_rna_files(path: Path) -> dict[str, pd.DataFrame]:
    ...
    if not data_dict:
        logger.warning(f"No DEG data found for {path}")
    return data_dict
...
def merge_FDR(fdrs: dict[str, pd.DataFrame]) -> pd.DataFrame:
    '''merges FDR dataframes into one'''
